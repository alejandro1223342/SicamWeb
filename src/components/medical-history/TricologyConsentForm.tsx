...
// TEMPLATES will be fetched from Drive dynamically

export default function TricologyConsentForm({ patientId, recordId, sessionId, data, onChange, onUploadingChange, readOnly = false }: ConsentFormProps) {
    ...
    // Fetch available templates from Drive
    useEffect(() => {
        const fetchTemplates = async () => {
            try {
                const response = await api.get('/drive/templates?specialty=Tricologia');
    ...
    useEffect(() => {
        const fetchFiles = async () => {
            ...
            try {
                // We use specialty="Tricologia" and folder="Consentimientos" for this section
                const response = await api.get(`/drive/patient/${patientId}?specialty=Tricologia&folder=Consentimientos&recordId=${recordId || ''}&sessionId=${sessionId || ''}`);
    ...
    const handleFileUpload = async (files: FileList | null) => {
        ...
        for (const file of Array.from(files)) {
            ...
            try {
                ...
                const response = await api.post(`/drive/upload?patientId=${patientId}&specialty=Tricologia&folder=Consentimientos&recordId=${recordId || ''}&sessionId=${sessionId || ''}`, formData, {
