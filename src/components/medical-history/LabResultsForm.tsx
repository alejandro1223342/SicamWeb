...
export default function LabResultsForm({ patientId, recordId, sessionId, data, onChange, onUploadingChange, readOnly = false }: LabResultsFormProps) {
    ...
    const handleFileSelection = async (fileList: FileList) => {
        ...
        for (const file of files) {
            ...
            try {
                const response = await api.post(`/drive/upload?patientId=${patientId}&specialty=Tricologia&folder=Examenes&recordId=${recordId || ''}&sessionId=${sessionId || ''}`, formData, {
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                ...
                                >
                                    <label style={{ cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
                                        ...
                                        <input type="file" multiple accept="image/*,application/pdf" style={{ display: 'none' }} onChange={e => e.target.files && handleFileSelection(e.target.files)} />
                                    </label>
                                </div>
